 ...
 buscarLugar: async (req, res) => {
    try {
      const options = {
        ...
        path: '/v3/places/search?ll=41.8781,-87.6298&query=restaurante&radius=5000&limit=5',
        headers: {
      ...
      const request = https.request(options, (response) => {
        ...
        response.on('end', async () => {
          ...
          if (data && data.results && data.results.length > 0) {
            ...
            res.status(200).json({
                success: true,
                message: 'Lugares encontrados con éxito',
                data: data.results
            });
          } else {
            res.status(404).json({
        ...
        response.on('end', () => {
          ...
          if (data && data.length > 0) {
            const photos = data.map(photo => {
              ...
              return 'https://fastly.4sqi.net/img/general/${photo.width}x${photo.height}${photo.prefix}${photo.photo_id}${photo.suffix}';
            });
            resolve(photos);
          } else {
